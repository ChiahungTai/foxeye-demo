...
function processOneFrame_RGBA(bitmap) {
  ...
  processOneFrame_RGBA_ArrayBuffer(bitmapBuffer, 0, bitmapBufferLength, bitmap.width, bitmap.height);
}

function processOneFrame_RGBA_ArrayBuffer(buffer, offset, length, width, height) {
  try {
    qrcode.decodeRGBAArrayBuffer(buffer, offset, length, width, height);
  } catch(e) {
    error(e);
  }
}
...
onmessage = function(event) {
  ...
  } else if (event.data.type == "process_one_frame_arraybuffer") {
    processOneFrame_RGBA_ArrayBuffer(event.data.buffer,
                                     event.data.offset,
                                     event.data.length,
                                     event.data.width,
                                     event.data.height);
